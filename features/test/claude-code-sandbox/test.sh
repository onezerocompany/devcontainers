...
check "firewall script exists" bash -c "test -f /usr/local/share/claude-code-sandbox/init-firewall.sh"
check "firewall script executable" bash -c "test -x /usr/local/share/claude-code-sandbox/init-firewall.sh"
...
if [ "$(id -u)" != "0" ]; then
    check "sudoers file exists" bash -c "test -f /etc/sudoers.d/claude-code-sandbox"
fi
